...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "army" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("army:client:UpdateBlips", -1, dutyPlayers)
end
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "army" and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent('army:client:SpawnSpikeStrip', src)
    end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        if type == "cone" then
            TriggerClientEvent("army:client:spawnCone", src)
        elseif type == "barrier" then
            TriggerClientEvent("army:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("army:client:spawnRoadSign", src)
        elseif type == "tent" then
            TriggerClientEvent("army:client:spawnTent", src)
        elseif type == "light" then
            TriggerClientEvent("army:client:spawnLight", src)
        elseif type == "delete" then
            TriggerClientEvent("army:client:deleteObject", src)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cuff", Lang:t("commands.cuff_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:CuffPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source)
    local src = source
    TriggerClientEvent("army:client:EscortPlayer", src)
end)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:JailPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("seizecash", Lang:t("commands.seizecash"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:SeizeCash", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("sc", Lang:t("commands.softcuff"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:CuffPlayerSoft", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cam", Lang:t("commands.camera"), {{name = "camid", help = Lang:t('info.camera_id')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:ActiveCamera", src, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:ImpoundVehicle", src, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
--    local Player = QBCore.Functions.GetPlayer(src)
--    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
--        TriggerClientEvent("army:client:ImpoundVehicle", src, true)
--    else
--        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
--    end
--end)
    ...
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney("bank", 500, "army-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if not OtherPlayer then return end
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            OtherPlayer.Functions.AddMoney("bank", 500, "army-lawyer-paid")
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "army" then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    exports['qb-management']:AddMoney('army', amount)
                else
        ...
        TriggerClientEvent('QBCore:Notify', source, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("anklet", Lang:t("commands.anklet"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:CheckDistance", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("ankletlocation", Lang:t("commands.ankletlocation"), {{name = "cid", help = Lang:t('info.citizen_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        ...
        if Target.PlayerData.metadata["tracker"] then
            TriggerClientEvent("army:client:SendTrackerLocation", Target.PlayerData.source, src)
        else
...
QBCore.Commands.Add("takedrivinglicense", Lang:t("commands.drivinglicense"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty then
        TriggerClientEvent("army:client:SeizeDriverLicense", source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_army_only"), 'error')
    end
end)
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.name ~= "army" or not Player.PlayerData.job.onduty then return end
...
RegisterNetEvent('army:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("army:client:TrackerMessage", requestId, msg, coords)
    TriggerClientEvent("qb-phone:client:addarmyAlert", requestId, alertData)
end)

QBCore.Commands.Add('911p', Lang:t("commands.army_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'army' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addarmyAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('army:client:armyAlert', v.PlayerData.source, coords, message)
        end
    end
end)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or Player.PlayerData.job.name == "army" or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
    Player.Functions.AddMoney("cash", tonumber(item.info.cash), "used-moneybag")
end)

-- Callbacks
QBCore.Functions.CreateCallback('army:server:isPlayerDead', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('army:GetPlayerStatus', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('army:IsSilencedWeapon', function(source, cb, weapon)
...
QBCore.Functions.CreateCallback('army:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "army" and v.PlayerData.job.onduty then
...
--QBCore.Functions.CreateCallback('army:GetImpoundedVehicles', function(_, cb)
--    local vehicles = {}
...
QBCore.Functions.CreateCallback('army:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('army:GetCops', function(_, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "army" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('army:server:IsarmyForcePresent', function(_, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "army" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        CreateThread(function()
            MySQL.query("DELETE FROM stashitems WHERE stash = 'armytrash'")
        end)
    end
end)

RegisterNetEvent('army:server:armyAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'army' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addarmyAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('army:client:armyAlert', v.PlayerData.source, coords, text)
        end
    end
end)

--RegisterNetEvent('army:server:TakeOutImpound', function(plate, garage)
--    local src = source
...
RegisterNetEvent('army:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and Player.PlayerData.job.name ~= "army") then return end

    TriggerClientEvent("army:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
end)

RegisterNetEvent('army:server:EscortPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if (Player.PlayerData.job.name == "army" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
        TriggerClientEvent("army:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('army:server:KidnapPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
        TriggerClientEvent("army:client:GetKidnappedTarget", EscortPlayer.PlayerData.source, Player.PlayerData.source)
        TriggerClientEvent("army:client:GetKidnappedDragger", Player.PlayerData.source, EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('army:server:SetPlayerOutVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("army:client:SetOutVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('army:server:PutPlayerInVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("army:client:PutInVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('army:server:BillPlayer', function(playerId, price)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "army" then return end

    OtherPlayer.Functions.RemoveMoney("bank", price, "paid-bills")
    exports['qb-management']:AddMoney("army", price)
    TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
end)

RegisterNetEvent('army:server:JailPlayer', function(playerId, time)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    TriggerClientEvent("army:client:SendToJail", OtherPlayer.PlayerData.source, time)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
end)

RegisterNetEvent('army:server:SetHandcuffStatus', function(isHandcuffed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
-- RegisterNetEvent('army:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings)
--     local src = source
...
--             if (Player.PlayerData.job.name == "army" and Player.PlayerData.job.onduty) then
--                 if street2 then
--                     TriggerClientEvent("112:client:SendarmyAlert", v, "flagged", {
--                         camId = camId,
--                         plate = plate,
...
RegisterNetEvent('army:server:SearchPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('army:server:SeizeCash', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    SearchedPlayer.Functions.RemoveMoney("cash", moneyAmount, "army-cash-seized")
...
RegisterNetEvent('army:server:SeizeDriverLicense', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('army:server:RobPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    Player.Functions.AddMoney("cash", money, "army-player-robbed")
    SearchedPlayer.Functions.RemoveMoney("cash", money, "army-player-robbed")
...
RegisterNetEvent('army:server:UpdateBlips', function()
    -- KEEP FOR REF BUT NOT NEEDED ANYMORE.
end)

RegisterNetEvent('army:server:spawnObject', function(type)
    local src = source
    local objectId = CreateObjectId()
    Objects[objectId] = type
    TriggerClientEvent("army:client:spawnObject", src, objectId, type, src)
end)

RegisterNetEvent('army:server:deleteObject', function(objectId)
    TriggerClientEvent('army:client:removeObject', -1, objectId)
end)

RegisterNetEvent('army:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
...
RegisterNetEvent('army:server:UpdateCurrentCops', function()
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    ...
    TriggerClientEvent("army:SetCopCount", -1, amount)
    updatingCops = false
end)
...
RegisterNetEvent('army:server:showFingerprint', function(playerId)
    local src = source
    TriggerClientEvent('army:client:showFingerprint', playerId, src)
    TriggerClientEvent('army:client:showFingerprint', src, playerId)
end)

RegisterNetEvent('army:server:showFingerprintId', function(sessionId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local fid = Player.PlayerData.metadata["fingerprint"]
    TriggerClientEvent('army:client:showFingerprintId', sessionId, fid)
    TriggerClientEvent('army:client:showFingerprintId', src, fid)
end)

RegisterNetEvent('army:server:SetTracker', function(targetId)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('army:client:SetTracker', targetId, false)
    else
        ...
        TriggerClientEvent('army:client:SetTracker', targetId, true)
    end
end)

RegisterNetEvent('army:server:SyncSpikes', function(table)
    TriggerClientEvent('army:client:SyncSpikes', -1, table)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("army:SetCopCount", -1, curCops)
    end
end)
