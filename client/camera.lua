...
-- Events
RegisterNetEvent('army:client:ActiveCamera', function(cameraId)
...
RegisterNetEvent('army:client:DisableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('army:client:EnableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('army:client:SetCamera', function(key, isOnline)
    ...
    elseif type(key) == 'number' then
        ...
        error('army:client:SetCamera did not receive the right type of key\nreceived type: ' .. type(key) .. '\nreceived value: ' .. key)
    end
end)
