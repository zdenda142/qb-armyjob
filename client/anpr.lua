		...
		if IsPedInAnyVehicle(myPed, false) then
			if GetPedInVehicleSeat(vehicle, -1) == myPed then
				if GetVehicleClass(vehicle) ~= 18 then
					...
					QBCore.Functions.TriggerCallback('army:IsPlateFlagged', function(result)
						if result then
							...
							TriggerServerEvent("army:server:FlaggedPlateTriggered", hasBeenBusted, plate, street1, street2, blipsettings)
						end
                    end, plate)
				end
			end
