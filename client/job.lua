...
function TakeOutImpound(vehicle)
    ...
    if coords then
		...
		SetVehicleDirtLevel(veh, 0.0)
                ...
                TriggerServerEvent('army:server:TakeOutImpound', vehicle.plate, currentGarage)
                closeMenuFull()
...
function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            local veh = NetToVeh(netId)
            SetCarItemsInfo()
            SetVehicleNumberPlateText(veh, Lang:t('info.army_plate')..tostring(math.random(1000, 9999)))
            SetEntityHeading(veh, coords.w)
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'army' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "army:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("army:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "army:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('army:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('army:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(_, cb)
    TriggerServerEvent('army:server:showFingerprintId', FingerPrintSessionId)
    cb("ok")
end)

RegisterNetEvent('army:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("army:server:SendEmergencyMessage", coords, message)
    TriggerEvent("army:client:CallAnim")
end)

RegisterNetEvent('army:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('army:client:CallAnim', function()
...
RegisterNetEvent('army:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerServerEvent("army:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
...
RegisterNetEvent('army:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "army" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('army:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
...
RegisterNetEvent("army:client:VehicleMenuHeader", function (data)
    MenuGarage(data.currentSelection)
    currentGarage = data.currentSelection
end)


RegisterNetEvent("army:client:ImpoundMenuHeader", function (data)
    MenuImpound(data.currentSelection)
    currentGarage = data.currentSelection
end)

RegisterNetEvent('army:client:TakeOutImpound', function(data)
    if inImpound then
...
RegisterNetEvent('army:client:TakeOutVehicle', function(data)
    if inGarage then
        local vehicle = data.vehicle
...
RegisterNetEvent('army:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-armyjob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("army:server:UpdateCurrentCops")
    TriggerServerEvent("army:server:UpdateBlips")
end)

RegisterNetEvent('qb-army:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("army:server:showFingerprint", playerId)
    else
        QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
end)

RegisterNetEvent('qb-army:client:openArmoury', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "army", authorizedItems)
end)

RegisterNetEvent('qb-army:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
            SetVehicleEngineOn(veh, true, true)
        end, Config.armyHelicopter, coords, true)
    end
end)

RegisterNetEvent("qb-army:client:openStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "armystash_"..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "armystash_"..QBCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('qb-army:client:openTrash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "armytrash", {
        maxweight = 4000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", "armytrash")
end)
...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and PlayerJob.name == "army" then
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "armytrash", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-army:client:scanFingerPrint")
                    break
                end
            else
                break
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inArmoury and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-army:client:openArmoury")
                    break
                end
            else
                break
...
local function heli()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-army:client:spawnHelicopter")
                    break
                end
            else
                break
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.name == "army" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['qb-target']:AddBoxZone("armyDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "armyDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("armyStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "armyStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-army:client:openStash",
                        icon = "fas fa-dungeon",
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['qb-target']:AddBoxZone("armyTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "armyTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-army:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        job = "army",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['qb-target']:AddBoxZone("armyFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                name = "armyFingerprint_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                maxZ = v.z + 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-army:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("armyArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                name = "armyArmory_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-army:client:openArmoury",
                        icon = "fas fa-swords",
                        label = "Open Armory",
                        job = "army",
                    },
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            if PlayerJob.name == 'army' and PlayerJob.onduty then
                ...
                exports['qb-menu']:showHeader({
                    {
                        header = Lang:t('info.evidence_stash', {value = currentEvidence}),
                        params = {
                            event = 'army:client:EvidenceStashDrawer',
                            args = {
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'army:client:ImpoundMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'army:client:VehicleMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
